...
import { IAuthService } from 'src/app/core/interfaces/auth/Iauth.service';
    ...
    login(email: string, password: string): Observable<ResponseDTO> {
        ...
        return this._configService.getUrlApplication().pipe(
            switchMap(url => {
                ...
                return this._httpService.post(url, "Authentication", null, loginPayload);
            }),
            catchError(error => {
        ...
        if (error.status === 429) {
            customError.userMessage = 'Demasiados intentos de inicio de sesión. Por favor, intente más tarde.';
