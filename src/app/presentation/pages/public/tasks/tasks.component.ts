...
import { TaskMapDataDTO, GetFilterTasksDTO } from 'src/app/core/data-transfer-object/App/task.dto';
...
export class TasksComponent implements OnInit, AfterViewInit {
  ...
  loadTasksData(): void {
    ...
    if (filters.taskStatusId !== null && filters.taskStatusId !== undefined) {
 ...
 clearFilters(): void {
  ...
  addNewTask(): void {
    ...
    this.showSuccessMessage('Función de crear tarea pendiente de implementar');
  }
  ...
  editTask(task: TaskMapDataDTO): void {
    ...
    this.showSuccessMessage('Función de editar tarea pendiente de implementar');
  }

  deleteTask(task: TaskMapDataDTO): void {
    if (confirm(`¿Está seguro de que desea eliminar la tarea "${task.titleTask}"?`)) {
      this.isLoading = true;
 ...
 toggleTaskCompletion(task: TaskMapDataDTO): void {
  ...
  const newStatusId = task.taskStatusId === 1 ? 2 : 1; // Cambiar entre 1 y 2
  
  this._taskUseCase.UpdateTask({
    taskId: task.taskId,
    ...
    next: (success) => {
      if (success) {
        ...
        this.showSuccessMessage(
          newStatusId === 2 ? 'Tarea completada' : 'Tarea marcada como pendiente'
        );
      } else {
        this.isLoading = false;
      }
    ...
    error: (error) => {
      this.isLoading = false;
      this.showErrorMessage('Error al actualizar el estado de la tarea');
